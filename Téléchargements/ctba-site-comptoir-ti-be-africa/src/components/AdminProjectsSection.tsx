...
export function AdminProjectsSection({
  ...
  const handleDrop = useCallback((e: React.DragEvent, dropIdx: number) => {
    ...
    // Reorder the full list: keep non-paged items in place, reorder paged section
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse min-w-[400px]">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="p-3 sm:p-4 font-semibold text-slate-700 dark:text-slate-200 text-sm">Projet</th>
                <th className="p-3 sm:p-4 font-semibold text-slate-700 dark:text-slate-200 text-sm">Catégorie</th>
                <th className="p-3 sm:p-4 font-semibold text-slate-700 dark:text-slate-200 text-sm">Année</th>
            ...
            <tbody>
              {paged.length === 0 ? (
                <tr>
                  <td colSpan={onReorder ? 5 : 4} className="p-8 text-center text-slate-500 dark:text-slate-400">
                    Aucun projet trouvé
                  </td>
                </tr>
              ) : (
                    ...
                    >
                      ...
                      <td className="p-3 sm:p-4">
                        <div className="flex items-center gap-2 sm:gap-3">
                          <img src={project.image} alt={project.title} className="w-8 h-8 sm:w-10 sm:h-10 rounded object-cover shrink-0" />
                          <div className="min-w-0">
                            <span className="font-medium text-slate-900 dark:text-white text-sm sm:text-base block truncate max-w-[120px] sm:max-w-none">{project.title}</span>
                            <span className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1 hidden sm:block">{project.description}</span>
                          </div>
                        </div>
                      </td>
                      <td className="p-3 sm:p-4">
                        <span className="px-2 py-1 rounded text-xs font-bold bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200">{project.category}</span>
                      </td>
                      <td className="p-3 sm:p-4 text-slate-600 dark:text-slate-300 text-sm">{project.date}</td>
                      <td className="p-3 sm:p-4 text-right whitespace-nowrap sticky right-0 z-10 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 shadow-[-8px_0_12px_-8px_rgba(0,0,0,0.15)]">
                        <button onClick={() => onEdit(project)} className="p-1.5 sm:p-2 text-slate-400 dark:text-slate-500 hover:text-brand-600 transition-colors"><Edit2 size={16} /></button>
                        <button onClick={() => onDelete(project.id, project.title)} className="p-1.5 sm:p-2 text-slate-400 dark:text-slate-500 hover:text-red-600 transition-colors"><Trash2 size={16} /></button>
                      </td>
                    </tr>
                  );
                })
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 dark:border-slate-700">
          <span className="text-sm text-slate-500 dark:text-slate-400">
            Page {page + 1} / {totalPages} ({filtered.length} projets)
          </span>
